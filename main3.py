...
# model.head = nn.Linear(in_features=192, out_features=10) #애는 출력층이 헤드임 왠진 몰루
# model.to(device)


# model = models.efficientnet_b4(pretrained=False)
# model.classifier[1] = torch.nn.Linear(in_features=1792,out_features=10)#450개로 분류하잖음
# model.to(device)
...
# model.head = nn.Linear(in_features=1024, out_features=10)
# model.to(device)
...
#optimizer = torch.optim.AdamW(model.head.parameters(), lr=0.0001) #이거는 위에 다른 모델에서 씀
optimizer = torch.optim.AdamW(model.parameters(), lr=0.0001) #이거는 레즈넷에서 씀

# lr scheduler
exp_lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=100,
                                                   gamma=0.1)
...
dfForAccuracy = pd.DataFrame(index=list(range(100)), #트레인의 에포치 값을 적어주세요
...
def validation(epoch, model, val_loader, criterion, device, num_epochs) :
    ...
    dfForAccuracy.loc[epoch, 'Accuracy'] = val_acc
...
if __name__ == "__main__" :
    ...
    #모델을 테스트함
    # model.load_state_dict(torch.load("./resnet50 5epoch 0.0001.pt", map_location=device))
